...
def data_generation_dense_2(covariance, num_control, num_treated, num_cov, pi, control_m = 0.1, treated_m = 0.9):
    ...
    def gen_xz(num_treated, num_control, dim):
        ...
        x1 = np.random.binomial(1,0.5,size = (num_control,num_cov))

        x2 = np.random.binomial(1,0.5,size = (num_treated,num_cov))
...
def run(pi):
    ...
    for i in range(1):
        ...
        num_control = 3000
        cov = 0.8
        df,x,z,d,r = data_generation_dense_2(cov,num_control, num_treated, 20, pi)
        df_sub.append(df)
        np.savetxt("data/x_linear_6000_multilevel_"+str(pi)+"_"+str(i)+".txt",x,fmt="%d")
        np.savetxt("data/z_linear_6000_multilevel_"+str(pi)+"_"+str(i)+".txt",z,fmt="%d")
        np.savetxt("data/d_linear_6000_multilevel_"+str(pi)+"_"+str(i)+".txt",d,fmt="%.8f")
        np.savetxt("data/r_linear_6000_multilevel_"+str(pi)+"_"+str(i)+".txt",r,fmt="%.8f")
...
if __name__ == '__main__':
    ...
    with Pool(min(10, len(pi_array))) as p:
        ...
        for pi, df_sub in drop_results:
            pickle.dump(df_sub, open('data/df_linear_6000_multilevel_'+str(pi), 'wb'))
