...
def run(pi):
    ...
    for i in range(10):
        ...
        df_sub.append(df)
        np.savetxt("data/x_linear_exp_"+str(pi)+"_"+str(i)+".txt",x,fmt="%d")
        np.savetxt("data/z_linear_exp_"+str(pi)+"_"+str(i)+".txt",z,fmt="%d")
        np.savetxt("data/d_linear_exp_"+str(pi)+"_"+str(i)+".txt",d,fmt="%d")
        np.savetxt("data/r_linear_exp_"+str(pi)+"_"+str(i)+".txt",r,fmt="%.8f")
...
if __name__ == '__main__':
    ...
    with Pool(min(10, len(pi_array))) as p:
        ...
        for pi, df_sub in drop_results:
            pickle.dump(df_sub, open('data/df_linear_exp_'+str(pi), 'wb'))
