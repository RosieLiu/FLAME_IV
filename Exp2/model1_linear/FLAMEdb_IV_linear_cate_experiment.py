...
def plot_treatment_comparison(x,y):
    ...
    np.savetxt("x_linear"+".txt",x,fmt="%.8f")
    np.savetxt("y_linear"+".txt",y,fmt="%.8f")
    print(mean_squared_error(x,y))

def get_num_groups(matching_res):
    num_trees = [0] * 2000
    ...
    for matching_level in matching_res:
        ...
        for matching_group in matching_level:
            group_idx = matching_group[-2]
            for idx in group_idx:
                num_trees[int(idx)] += num_trees[int(idx)] + len(group_idx) - 1

    print(num_trees)
    return num_trees

def get_num_matched(matching_res):
    num_matched_dict = {}
    ...
    for matching_level in matching_res:
        if matching_level is None:
            continue
        for matching_group in matching_level:
            group_idx = matching_group[1]
            match_idx = matching_group[2]
            for idx in match_idx:
                key = int(idx)
                if key not in num_matched_dict:
                    num_matched_dict[key] = set()
                for elem in group_idx:
                    if int(elem) != key:
                        value = int(elem)
                        num_matched_dict[key].add(value)


    num_matched = [0] * 2000
    for key in num_matched_dict:
        num_matched[key] = len(num_matched_dict[key])

    return num_matched

def get_weighted_num_matched(matching_res):
    num_matched_weight_dict = [[0] * 2000] * 2000
        ...
        for matching_group in matching_level:
            ...
            for idx in match_idx:
                ...
                for elem in group_idx:
                    value = int(elem)
                    if value != key:
                        num_matched_weight_dict[key][value] += 1
                        num_matched_weight_dict[value][key] += 1

    print(num_matched_weight_dict)
    return num_matched_weight_dict
...
def run(pi):
    ...
    df_all= pickle.load(open('data/df_linear_exp_'+str(pi), "rb"))
...
if __name__ == '__main__':
    ...
    df_all= pickle.load(open('data/df_linear_exp_'+str(pi), "rb"))
    df = df_all[i]
    np.savetxt("data/df_linear_"+str(pi)+"_"+str(i)+".txt",df,fmt="%d")
