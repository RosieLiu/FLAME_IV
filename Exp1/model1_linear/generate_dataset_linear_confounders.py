...
def run(pi):
    ...
    for i in range(50):
        ...
        df_sub.append(df)
        np.savetxt("data50/x_linear_50_multilevel_co_"+str(pi)+"_"+str(i)+".txt",x,fmt="%d")
        np.savetxt("data50/z_linear_50_multilevel_co_"+str(pi)+"_"+str(i)+".txt",z,fmt="%d")
        np.savetxt("data50/d_linear_50_multilevel_co_"+str(pi)+"_"+str(i)+".txt",d,fmt="%.8f")
        np.savetxt("data50/r_linear_50_multilevel_co_"+str(pi)+"_"+str(i)+".txt",r,fmt="%.8f")
...
if __name__ == '__main__':
    ...
    with Pool(min(10, len(pi_array))) as p:
        ...
        for pi, df_sub in drop_results:
            pickle.dump(df_sub, open('data50/df_linear_50_multilevel_co_'+str(pi), 'wb'))
