...
def data_generation_dense_2(covariance, num_control, num_treated, num_cov_important, num_cov_unimportant, pi, control_m = 0.1, treated_m = 0.9):
    ...
    def gen_xz(num_treated, num_control, dim):
        ...
        rou = np.random.normal(0.1, 0.01, 2).reshape((2,1))
        z_ = np.matmul(x[:,15:17],rou).reshape((num_treated + num_control,1))
        threshold = statistics.median(list(z_))
        z = np.asarray([0 if e < threshold else 1 for e in z_[:,0]]).reshape((num_treated + num_control,1))
        return x, z
    ...
    second = construct_sec_order(x[:,:5])
    ...
    Rij = np.add(Rij, np.array(treatment_eff_sec).reshape((num_treated + num_control,1)) * 20)
...
def run(pi):
    ...
    for i in range(50):
        ...
        num_control = 1000
        cov = 0.8
        df,x,z,d,r = data_generation_dense_2(cov,num_control, num_treated, 5, 12, pi)
        df_sub.append(df)
        np.savetxt("data/x_nonlinear_1000_multilevel_"+str(pi)+"_"+str(i)+".txt",x,fmt="%d")
        np.savetxt("data/z_nonlinear_1000_multilevel_"+str(pi)+"_"+str(i)+".txt",z,fmt="%d")
        np.savetxt("data/d_nonlinear_1000_multilevel_"+str(pi)+"_"+str(i)+".txt",d,fmt="%.8f")
        np.savetxt("data/r_nonlinear_1000_multilevel_"+str(pi)+"_"+str(i)+".txt",r,fmt="%.8f")
...
if __name__ == '__main__':
    ...
    with Pool(min(10, len(pi_array))) as p:
        ...
        for pi, df_sub in drop_results:
            pickle.dump(df_sub, open('data/df_nonlinear_1000_multilevel_'+str(pi), 'wb'))
