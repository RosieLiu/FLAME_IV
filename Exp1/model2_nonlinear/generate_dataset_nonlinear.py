...
def run(pi):
    ...
    for i in range(10):
        ...
        num_control = 50
        cov = 0.8
        df,x,z,d,r = data_generation_dense_2(cov,num_control, num_treated, 5, 10, pi)
        df_sub.append(df)
        np.savetxt("data/x_nonlinear_50_multilevel_"+str(pi)+"_"+str(i)+".txt",x,fmt="%d")
        np.savetxt("data/z_nonlinear_50_multilevel_"+str(pi)+"_"+str(i)+".txt",z,fmt="%d")
        np.savetxt("data/d_nonlinear_50_multilevel_"+str(pi)+"_"+str(i)+".txt",d,fmt="%.8f")
        np.savetxt("data/r_nonlinear_50_multilevel_"+str(pi)+"_"+str(i)+".txt",r,fmt="%.8f")
...
if __name__ == '__main__':
    ...
    with Pool(min(10, len(pi_array))) as p:
        ...
        for pi, df_sub in drop_results:
            pickle.dump(df_sub, open('data/df_nonlinear_50_multilevel_'+str(pi), 'wb'))
