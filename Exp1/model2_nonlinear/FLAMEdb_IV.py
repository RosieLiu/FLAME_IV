...
def data_generation_dense_2(covariance, num_control, num_treated, num_cov_important, num_cov_unimportant, pi, control_m = 0.1, treated_m = 0.9):
    ...
    df = df.reset_index()
    for i in range(num_cov_important + num_cov_unimportant):
...
def score_tentative_drop_c(cov_l, c, db_name, holdout_df, thres = 0, tradeoff = 0.1):
    ...
    covs_to_match_on = list(set(cov_l) - {c}) # the covariates to match on
...
def run(pi):
    ...
    table_name = 'flame_1_' + str(int(100*pi))
    
    LATE_list = []  
    
    df_all= pickle.load(open('data/df_nonlinear_1000_multilevel_'+str(pi), "rb"))
    ...
    for i in range(50):
        ...
        #print(df.shape)
        num_cov = 30
        holdout_df,x,z,d,r = data_generation_dense_2(cov,1000,1000,5,12,pi)  
        df.to_sql(table_name, engine)
        res = run_db(cur, conn,table_name, holdout_df, 17)
        LATE = get_LATE(res)
...
if __name__ == '__main__':
    ...
    with Pool(min(8, len(pi_array))) as p:
        drop_results = p.map(run, pi_array)
        for late_list in drop_results:
            late_list = [x for x in late_list if x != None]
