...
def run(pi):
    ...
    for i in range(1000):
        ...
        df_sub.append(df)
        np.savetxt("data/x_linear_1000_multilevel_"+str(pi)+"_"+str(i)+".txt",x,fmt="%d")
        np.savetxt("data/z_linear_1000_multilevel_"+str(pi)+"_"+str(i)+".txt",z,fmt="%d")
        np.savetxt("data/d_linear_1000_multilevel_"+str(pi)+"_"+str(i)+".txt",d,fmt="%.8f")
        np.savetxt("data/r_linear_1000_multilevel_"+str(pi)+"_"+str(i)+".txt",r,fmt="%.8f")
        pickle.dump(df, open('data/df_linear_1000_multilevel_'+str(pi)+"_"+str(i), 'wb'))
    return pi,df_sub
...
if __name__ == '__main__':
    ...
    with Pool(min(10, len(pi_array))) as p:
        ...
        for pi, df_sub in drop_results:
            pickle.dump(df_sub, open('data/df_linear_15000_multilevel_'+str(pi), 'wb'))
        """  
