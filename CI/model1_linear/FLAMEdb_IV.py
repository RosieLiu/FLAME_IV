...
def get_LATE_and_CI(df,res,total_num):
    ...
    if not df_all.empty:
        ...
        df_all['weighted_CACE_t'] = df_all['CACE_t'] * df_all['count']
        sum_all = df_all.sum(axis = 0)
        total_num = sum_all['count']
        ITT_y = sum_all['weighted_CACE_y'] * 1.0 / sum_all['count']
        ITT_t = sum_all['weighted_CACE_t'] * 1.0 / sum_all['count']
        ATE = ITT_y/ITT_t
    ...
    for level in match_list:
        ...
        for idx, row in level.iterrows():
            ...
            s_0 = ((df_sub['outcome'] * (1 - df_sub['iv']) - y_z_0 * 1.0 / n_0) ** 2).sum() * 1.0 / n_0
            s_1 = ((df_sub['outcome'] * df_sub['iv'] - y_z_1 * 1.0 / n_1) ** 2).sum() * 1.0 / n_1
            r_1 = ((df_sub['treated'] * df_sub['iv'] - t_z_1 * 1.0 / n_1) ** 2).sum() * 1.0 / n_1
            var_y += (s_1 / n_1 + s_0 / n_0) * ((n_1 + n_0) ** 2) / (total_num ** 2)
            var_t += (r_1 / n_1) * ((n_1 + n_0) ** 2) / (total_num ** 2)
            cov_grp = ((df_sub_1['outcome'] * df_sub_1['iv'] - y_z_1 * 1.0 / n_1) * (df_sub_1['treated'] * df_sub_1['iv'] - t_z_1 * 1.0 / n_1)).sum() / (n_1 ** 2)
...
def run(pi):
    ...
    for i in range(1000):
        ...
        cov = 0.8
        #df = df_all[i]
        df = pickle.load(open('data/df_linear_1000_multilevel_'+str(pi)+'_'+str(i), "rb"))
        holdout_df,x,z,d,r = data_generation_dense_2(cov,1000,1000,8,2,pi)  
        df.to_sql(table_name, engine)
        res = run_db(cur, conn,table_name, holdout_df, 10)
        late,i1,i2 = get_LATE_and_CI(df,res,df.shape[0])
        if i1 <= 10 and i2 >= 10:
            coverage += 1
